...
namespace SimuCoin
{
    public class NoForm : HttpClient, IDisposable
    {
        ...
        public async Task PluginNoFormLogin(string username, string password)
        {
        ...
        private async void DisplayBalance() // Update the SimuCoin balance and claim any available rewards
        {
            try
            {
                var pageContent = await GetPageContent(BalanceUrl); // Get the balance page content
                //UpdateTime(pageContent); // Update the time with the next subscription bonus time
                ...
                if (!string.IsNullOrEmpty(claimAmount))
                {
                    ...
                    PluginInfo.Coin?.EchoText($"Subscription Reward: {claimAmount} Free SimuCoins");
                    await ClaimReward();
                }
                else
                {
